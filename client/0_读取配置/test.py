...
def run_concurrently(task_list):
    ...
    filename = task_list
    filename_arr = filename.split(",")
    log_warning.warning(SEP_ASTERISK + '开始这是一个并行队列的开始' + SEP_ASTERISK)
    # 对象列表
...
if __name__ == '__main__':
    ...
    try:
        ...
        filename_script_log_info = './tmp_script_check.log_info_' + filename_suffix
        filename_script_log_warning = './tmp_script_check.log_warning_' + filename_suffix
        setup_logger('log_info', filename_script_log_info)
        setup_logger('log_warning', filename_script_log_warning)
        ...
        log_warning.warning(SEP_ASTERISK * 2 + '完成配置文件中的任务:%s' + SEP_ASTERISK * 2, str(filename))
